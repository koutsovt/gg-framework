...
- Pointer activation and popup dismissal must not leave a false selected or active treatment. DOM focus may remain, and text-entry controls or user/browser preferences may legitimately retain visible focus after a click. Separate focus from material decoration and genuine selected, expanded, and error states; never suppress visible keyboard focus globally. Focus/control-edge changes must pass the applicable interaction regression matrix in `craft-rulings.md`, including pointer → popup close → Space/Enter → Tab, overlay focus return, and combined border/shadow/outline inspection. Screenshots alone cannot pass this check.
...
- Select, dropdown, and combobox indicators must have a deliberate logical trailing inset and reserved content padding for the icon width and gap. Native text padding is not proof of arrow positioning. Reuse one shared control pattern: either verified native appearance or a native select with its browser arrow removed and one correctly inset decorative replacement. They fail when the indicator touches the edge, overlaps text or adjacent actions, duplicates the native indicator, or creates a dead pointer zone; verify long values, narrow widths, zoom, RTL, forced colors, and clicking the arrow itself.
